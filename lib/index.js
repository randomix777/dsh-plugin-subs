 ...
 * Respects `expires_in`, `interval`, and `slow_down` from the device response.
 * Supports AbortSignal for cancellation.
 */
async function qwenPollDeviceToken(deviceCode, verifier, signal) {
	const body = qwenEncodeUrlEncoded({
	...
	while (true) {
		...
		let response;
		try {
			response = await fetch(QWEN_TOKEN_URL, {
				method: "POST",
				headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" },
				body,
				signal,
		...
		if (!response.ok) {
			...
			if (response.status === 429) {
				// slow_down: double the interval, cap at 30s.
				pollIntervalMs = Math.min(pollIntervalMs * 2, 30_000);
				continue;
			}
...
async function runQwenDevicePoll(flow, controller) {
	try {
		const tokenData = await qwenPollDeviceToken(flow.deviceCode, flow.pkce.verifier, flow.signal);
...
var SubscriptionsAuthController = class {
	...
	cancel(provider) {
		...
		if (deviceFlow !== void 0) {
			deviceFlow.signal?.abort(new Error("login cancelled"));
