...
async function qwenPollDeviceToken(deviceCode, verifier) {
	const body = qwenEncodeUrlEncoded({
	...
	while (true) {
		...
		if (!response.ok) {
			...
			if (response.status === 400 && json?.error === "authorization_pending") { await new Promise(r => setTimeout(r, 2000)); continue; }
		...
		const data = await response.json();
		if (data.error) throw new Error("qwen token poll error: " + (data.error_description || data.error));
		return data;
	}
}
async function exchangeQwenCode(pkce) {
	const auth = await qwenRequestDeviceAuthorization(pkce);
	await openBrowser(auth.verification_uri_complete);
	const tokenData = await qwenPollDeviceToken(auth.device_code, pkce.verifier);
	...
	return {
		...
		expiresAt: tokenData.expires_in ? Date.now() + tokenData.expires_in * 1000 : Date.now() + 3600_000,
...
var SubscriptionsAuthController = class {
	...
	async login(provider) {
		...
		if (provider === "qwen") {
			try {
				...
				const session = await exchangeQwenCode(pkce);
				await this.persist("qwen", session);
				this.lastError.delete("qwen");
				this.onAuthChanged("qwen");
				return { authorizeUrl: "" };
			} catch (error) {
				throw new Error("Qwen OAuth failed: " + (error instanceof Error ? error.message : String(error)));
			}
		}
	...
	exchange(provider, code, attempt) {
		switch (provider) {
			...
			case "qwen": return exchangeQwenCode(attempt.pkce);
